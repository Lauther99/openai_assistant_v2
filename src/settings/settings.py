...
class Template:
    ...
    general_chain_template: str = """
        Your name is M-Assistant, a very helpful assistant.\n{task}
    """

    memory_template: str = """
        Your task is to rogressively summarize the lines of conversation provided, adding onto the previous summary returning a new summary. 
    ...
    requirement_chain_template: str = (
        """
        Your task is to find what is the last thing the human is requiring from AI in the next conversation:
        Conversation
        '''
        {conversation}
        '''
        ...
        Do not hallucinate or try to predict the future AI/Human message in the conversation. 
        Do not give your advices or recommendations in the response.
        Do not answer directly the user requirements.
        If the user's last message is related to previous messages for the request, put necessary information from previous messages in the final request of your response.
        If there are no request, answer with the last user intention or action
        
        Your answer have to respond the question: What is the last thing the human is requiring/doing in the conversation?
        
        Use strictly the following format to respond:
        summary: A detailed summary of the conversation
        request: Your brief but detailed answer
        """
    )
    
    # requirement_chain_template: str = (
    #     """
    #     Your task is to find the last human intention from a given conversation summary.
    #     Do not hallucinate or try to predict the next intention in the conversation. Work exclusively with the summary information.
        
    #     Use the following format to respond:
    #     summary: The given summary 
    #     request: The last human intention
    ...
    keywords_chain_template: str = (
        """
        Your task is to find keywords in the next 'user_requirement'.
        ...
        Also see examples to have an idea of how must be your answer:
        EXAMPLES:
        """
    )
    ...
    simple_classifier_chain_template: str = """
        Your task is to classify the request into one of the following categories: simple/complex
        simple: 
        ...
        The next is information you have to know before classify, there is a measurement system database, but you do not have access to this database.
    ...
    greeting_chain_template: str = (
        """
        Youy task is to receive and greet people.
    ...
    translator_template: str = (
        """
        Your task is to translate, follow this steps:
        
        First, detect language in text 1:
        Text 1: '''{user_input}'''
        
        Second, detect language in text 2:
        Text 2: '''{actual_answer}'''
        
        Third, if languages are different you have to translate the text 2 into the text 1 language else if the languages are the same, then response with same text 2.

        Use the following format to respond:
        detected_language: english/spanish/portuguesse, etc
        response: answer translated into the detected language if necessary
        """
    )
